...
{
    public class MealyDecodeTransform : ICryptoTransform, IDisposable
    {
        ...
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            var written = 0;
            while (inputCount > 0) {
                outputBuffer[outputOffset++] = Decode(inputBuffer[inputOffset++]);
                inputCount--;
                written++;
            }
            return written;
        ...
        byte Decode(byte codeUnit)
        {
            var lo = Seek(Lo(codeUnit));
            var hi = Seek(Hi(codeUnit));
            return Make(lo, hi);
        }
