...
{
    public class DijkstraShortestPathsAlgorithm : IShortestPaths<int, WeightedDigraph.Edge, double>
    {
        private readonly WeightedDigraph.Edge[] _edgeTo;
        private readonly double[] _distanceTo;

        public DijkstraShortestPathsAlgorithm(WeightedDigraph graph, int s) {
            ...
            var pq = new IndexedPriorityQueue<double>(V);
            pq.Enqueue(s, 0d);
            while (pq.Count > 0)
                Relax(graph, pq.Dequeue(), pq);
        }

        private void Relax(WeightedDigraph graph, int v, IndexedPriorityQueue<double> pq) {
            foreach (var e in graph.Edges(v)) {
                ...
                if (_distanceTo[w] > weight) {
                    ...
                    _edgeTo[w] = e;
                    if (pq.Contains(w))
                        pq.ChangeValue(w, _distanceTo[w]);
                    else
                        pq.Enqueue(w, _distanceTo[w]);
                }
            }
        }
