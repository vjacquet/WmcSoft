...
namespace WmcSoft.Reflection
{
    public static class ReflectionHelper
    {
        ...
        {
            if (qualifiedTypeName == null) throw new ArgumentNullException(nameof(qualifiedTypeName));
            var strArray = qualifiedTypeName.Split(new char[] { ',' });
        ...
        public static Type LoadType(string className, bool callingAssembly)
        {
            ...
            var type = (callingAssembly)
                ? Assembly.GetCallingAssembly().GetType(className.Trim(), false, true)
                : Type.GetType(className.Trim(), false, true);
            if (type != null)
                return type;

            var strArray = className.Split(new char[] { ',' });
            var assembly = Assembly.Load(GetAssemblyName(className));
            if (assembly == null)
