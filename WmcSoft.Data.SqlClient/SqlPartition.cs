...
WHERE name = '{name}'";
        ...
        public int GetLowerBound()
        {
            using (var loan = _connection.Borrow()) {
                return _connection.ExecuteScalar<int>(QueryPartitions("MIN(value)"));
            }
        }

        public int GetUpperBound()
        {
            const string query = @"SELECT MAX(value) FROM sys.partition_range_values v
JOIN sys.partition_functions f ON f.function_id=v.function_id
WHERe name = @name";
        ...
        public int DiscardUntil(int boundary, params string[] tables)
        {
            ...
            const string query = @"SELECT boundary_id, value, boundary_value_on_right FROM sys.partition_range_values v
JOIN sys.partition_functions f ON f.function_id=v.function_id
